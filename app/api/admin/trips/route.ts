...
export async function GET(req: Request) {
  try {
    ...
    const trips = await prisma.trip.findMany({
      where: { vehicle: { is: { ownerId: Number(admin.userId) } } },
...
export async function POST(req: Request) {
  try {
    ...
    const ownerVehicle = await prisma.vehicle.findFirst({
      where: { id: Number(vehicleId), ownerId: Number(admin.userId) },
      select: { id: true },
    });
    if (!ownerVehicle) return NextResponse.json({ error: "Vehicle not found for this owner" }, { status: 404 });
