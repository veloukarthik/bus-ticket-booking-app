...
export async function GET(req: Request) {
  ...
  const vehicles = await prisma.vehicle.findMany({ where: { organizationId: admin.organizationId } });
...
export async function POST(req: Request) {
  ...
  const v = await prisma.vehicle.create({
    data: { name, number, capacity: Number(capacity) || 0, organizationId: admin.organizationId },
  });
